...
\vspace{5mm}
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex']
    \fill [green!30] (-3,2.4) rectangle (8.7,1.1);
...
%Whereas a pointer contains an address to an area in memory, a fat pointer contains an address and additional information.
...
%Store and compare instructions terminate the chains leading to both of their operands, the return instruction terminates the chain of its operand and a function call terminates the chain of all of its parameters and initiates one for its return value.
...
Because LLVM module passes run on a single compilation unit, issues arise when using functions declared in other source files that the user has still written.
...
%The LLVM IR will contain a constant integer (indistinguishable from other all other constant integers) representing the size of the original struct, though the actual struct used will be the fat pointer version, that requires more memory.
...
I implemented a lookup table using \textit{uthash}, a hash table implementation written in C and released under the revised BSD license. 
...
%(though it still a linear lookup, that would take longer than an array access)
...
%As an optimisation, a simplified form of not-null analysis was implemented, to remove some of the redundant null checks.
%
