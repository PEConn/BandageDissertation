...
In this section, a number of these other pointer safety systems will be summarized and used to highlight the common gaps this work hopes to address.
...
Proposes hardware checks instead - the hardware bounded pointer, which is essentially a fat pointer, but the metadata is dealt with by hardware transparently to the programmer.
Registers are upgraded to triples of \verb!{value, base, bound}!, where \verb!base! and \verb!bound! are 0 for non-pointers.
Conclusion states lower overheads and better binary compatibility, with 100\% precision and recall on tests.
A suggestion was made to use HardBound for the CCured \verb!SEQ! and \verb!WILD! pointers, but not on \verb!SAFE! ones.
...
\textit{Intel MPX (Memory Protection Extensions)} \cite{mpx,intelMpxSpec} are a set of extensions to the x86 instruction set designed to check pointer references that may be exploited for security exploits.
They require compiler, runtime library and operating system support.
By embedding these checks in hardware, much of the overhead of pointer checking is reduced.
